//+------------------------------------------------------------------+
//| PhoenixTrendTraderv0.008 EA
//|
//|    Generated by StrategyQuant EA Wizard version 2.0.1
//|    Generated at Thu Jan 10 21:41:00 EST 2019
//+------------------------------------------------------------------+
...
int init() {
   ...
   if(DisplayInfoPanel) {
      ...
      ObjectSetText("line1", "PhoenixTrendTraderv0.008 EA", 9, "Tahoma", LabelColor);
...
//+------------------------------------------------------------------------------------------+
//+ Phoenix Trend Trader Library v0.008
//+
//+------------------------------------------------------------------------------------------+
//
...
void UpdateRobotManagers()
{
   ...
   {
      stopPrice = GetLowestPriceOfOpenClose(NULL,wbm.WaveBreakTimeframe,10);
      MoveStopsOnAllOpenOrders(stopPrice,true);   
   }
   
   if(wboem.TradeDirection==directionByIntValue.ShortIntValue)
   {
      stopPrice = GetHighestPriceOfOpenClose(NULL,wbm.WaveBreakTimeframe,10);
      MoveStopsOnAllOpenOrders(stopPrice,true);   
   }
...
int GetTrendFromEMA()
{
   ...
   for(int shift=1;shift <=5;shift++)
   {
      ...
      if((EMA1 > EMA2 && EMA2 > EMA3)){trendSum = trendSum + 1;}
      if((EMA1 < EMA2 && EMA2 < EMA3)){trendSum = trendSum - 1;}
   }

   if(trendSum>=4){trend=directionByIntValue.LongIntValue;}
   if(trendSum<=-4){trend=directionByIntValue.ShortIntValue;}
...
void UpdateWaveBreakOrderEntryManager()
{
   ...
   VerboseLog("Pivot Line Type: " + wboem.PivotLineType);
   
   //Okay to go long?
   if(wboem.ThreeEMATrendDirection == directionByIntValue.LongIntValue)
   {
      if(wboem.BreakoutPriceFlag)
      {
         if(wboem.TriangleBreakoutDirection == directionByIntValue.LongIntValue)
         {
            if(wboem.CurrentOpenInPositionFlag)
            {
               if(wboem.CurrentCandleColorCorrectFlag)
               {
                 if(wboem.TrianglePointOneNotViolatedFlagOn)
                  {
                    if(wboem.PriceExtremeReachedFlag)
                    {
                     if(wboem.PivotLineType=directionByIntValue.LongIntValue)
                     {
   ...
   //Okay to go short?
   if(wboem.ThreeEMATrendDirection == directionByIntValue.ShortIntValue)
   {
      if(wboem.BreakoutPriceFlag)
      {
         if(wboem.TriangleBreakoutDirection == directionByIntValue.ShortIntValue)
            ...
            if(wboem.CurrentOpenInPositionFlag)
            {
               if(wboem.CurrentCandleColorCorrectFlag)
               {
                 if(wboem.TrianglePointOneNotViolatedFlagOn)
                 {
                   if(wboem.PriceExtremeReachedFlag)
                   {    
                   if(wboem.PivotLineType = directionByIntValue.ShortIntValue)
                   {                
   ...
   if(wboem.WaveBreakOrderEntryManagerFlagOn)
   {
      ...
      debugObject.Add("  Three EMA Trend: " + wboem.ThreeEMATrendDirection + "\n");
      debugObject.Add("  Breakout Price Flag: " + wboem.BreakoutPriceFlag + "\n");
      debugObject.Add("  Triangle Breakout Direction: " + wboem.TriangleBreakoutDirection + "\n");
      debugObject.Add("  Pivot Line Type: " + wboem.PivotLineType + "\n");
      debugObject.Add("  Current Open In Position Flag: " + wboem.CurrentOpenInPositionFlag + "\n");
      debugObject.Add("  Current Candle Color Correct Flag: " + wboem.CurrentCandleColorCorrectFlag + "\n");
      debugObject.Add("  Triangle Point One Not Violated Flag On: " + wboem.TrianglePointOneNotViolatedFlagOn + "\n");
      debugObject.Add("  Price Extreme ReachedFlag: " + wboem.PriceExtremeReachedFlag + "\n");
