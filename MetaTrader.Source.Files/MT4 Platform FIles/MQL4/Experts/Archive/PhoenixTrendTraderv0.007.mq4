//+------------------------------------------------------------------+
//| PhoenixTrendTraderv0.007 EA
//|
//|    Generated by StrategyQuant EA Wizard version 2.0.1
//|    Generated at Thu Jan 10 18:23:00 EST 2019
//+------------------------------------------------------------------+
...
int init() {
   ...
   if(DisplayInfoPanel) {
      ...
      ObjectSetText("line1", "PhoenixTrendTraderv0.007 EA", 9, "Tahoma", LabelColor);
...
//+------------------------------------------------------------------------------------------+
//+ Phoenix Trend Trader Library v0.007
//+
//+------------------------------------------------------------------------------------------+
//
...
double GetEMA(int timeFrame,int maPeriod)
{
   return(iMA(NULL,timeFrame,maPeriod,0,MODE_EMA,0,1));
}

int GetTrendFromEMA()
{
   double EMA1 = GetEMA(temamam.ThreeEMATrendDirection_Timeframe,temamam.ThreeEMATrendDirection_Shortest);
   double EMA2 = GetEMA(temamam.ThreeEMATrendDirection_Timeframe,temamam.ThreeEMATrendDirection_Middle);
   double EMA3 = GetEMA(temamam.ThreeEMATrendDirection_Timeframe,temamam.ThreeEMATrendDirection_Longest);

   //0 = Consolidation, 1 = Long, -1 = Short
   int trend = directionByIntValue.NeutralIntValue;    
   
   if((EMA1 > EMA2 && EMA2 > EMA3)  ){trend=directionByIntValue.LongIntValue;}
   if((EMA1 < EMA2 && EMA2 < EMA3)  ){trend=directionByIntValue.ShortIntValue;}
...
bool GetPriceExtremeReachedFlag()
{
   ...
   VerboseLog("Long Check - Close[0]: " + Close[0] + " Highest Price: " + GetHighestPrice(NULL,wbm.WaveBreakTimeframe,1,tboo[0].Price1) + " Flag: " + Close[0] > GetHighestPrice(NULL,wbm.WaveBreakTimeframe,1,tboo[0].Price1));
   VerboseLog("Short Check - Close[0]: " + Close[0] + " Lowest Price: " + GetLowestPrice(NULL,wbm.WaveBreakTimeframe,1,tboo[0].Price1)+ " Flag: " + Close[0] < GetLowestPrice(NULL,wbm.WaveBreakTimeframe,1,tboo[0].Price1));
   
   if(wboem.TriangleBreakoutDirection==directionByIntValue.LongIntValue)
   {
      if(Close[0] > GetHighestPrice(NULL,wbm.WaveBreakTimeframe,1,tboo[0].Price1))
      {
         flag=true;
      }
   }
   ...
   if(wboem.TriangleBreakoutDirection==directionByIntValue.ShortIntValue)
   {
      if(Close[0] < GetLowestPrice(NULL,wbm.WaveBreakTimeframe,1,tboo[0].Price1))
      {
         flag=true;
      }
   } 
